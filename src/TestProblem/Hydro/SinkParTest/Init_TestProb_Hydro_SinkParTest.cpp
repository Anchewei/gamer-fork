...
static double SinkParTest_Dens_Contrast; // The density contrast between the
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Validate
// Description :  Validate the compilation flags and runtime parameters for this
// test problem
//
// Note        :  None
//
...
void Validate() {
  ...
  if (MPI_Rank == 0)
    Aux_Message(stdout, "   Validating test problem %d ...\n", TESTPROB_ID);
...
// Description :  Read problem-specific runtime parameters from Input__TestProb
// and store them in HDF5 snapshots (Data_*)
//
// Note        :  1. Invoked by SetParameter() to read parameters
//                2. Invoked by Output_DumpData_Total_HDF5() using the function
//                pointer Output_HDF5_InputTest_Ptr to store parameters
//                3. If there is no problem-specific runtime parameter to load,
//                add at least one parameter
//                   to prevent an empty structure in HDF5_Output_t
//                   --> Example:
//                       LOAD_PARA( load_mode, "TestProb_ID", &TESTPROB_ID,
//                       TESTPROB_ID, TESTPROB_ID, TESTPROB_ID );
//
// Parameter   :  load_mode      : Mode for loading parameters
//                                 --> LOAD_READPARA    : Read parameters from
//                                 Input__TestProb
//                                     LOAD_HDF5_OUTPUT : Store parameters in
//                                     HDF5 snapshots
//                ReadPara       : Data structure for reading parameters (used
//                with LOAD_READPARA) HDF5_InputTest : Data structure for
//                storing parameters in HDF5 snapshots (used with
//                LOAD_HDF5_OUTPUT)
//
// Return      :  None
//-------------------------------------------------------------------------------------------------------
...
# ifndef SUPPORT_HDF5
  if (load_mode == LOAD_HDF5_OUTPUT)
    Aux_Error( ERROR_INFO, "please turn on SUPPORT_HDF5 in the Makefile for load_mode == LOAD_HDF5_OUTPUT !!\n" );
# endif

  if (load_mode == LOAD_READPARA && ReadPara == NULL)
    Aux_Error( ERROR_INFO, "load_mode == LOAD_READPARA and ReadPara == NULL !!\n" );

  if (load_mode == LOAD_HDF5_OUTPUT && HDF5_InputTest == NULL)
    Aux_Error( ERROR_INFO, "load_mode == LOAD_HDF5_OUTPUT and HDF5_InputTest == NULL !!\n" );

  // add parameters in the following format:
  // --> note that VARIABLE, DEFAULT, MIN, and MAX must have the same data type
  // --> some handy constants (e.g., NoMin_int, Eps_float, ...) are defined in
  // "include/ReadPara.h"
...
void Load_Turbulence_SinkParTest() {
  ...
  Table_Rescaled_VelX = new double[tur_table_NBin];
  Table_Rescaled_VelY = new double[tur_table_NBin];
  Table_Rescaled_VelZ = new double[tur_table_NBin];
...
void SetParameter() {
  ...
  if (MPI_Rank == 0)
    Aux_Message(stdout, "   Setting runtime parameters ...\n");
  ...
  const double End_T_Default = 1.3065165e+12 / UNIT_T; // 2.5 tff
  ...
  if (MPI_Rank == 0)
    Aux_Message(stdout, "   Setting runtime parameters ... done\n");
...
//
// Note        :  1. This function may also be used to estimate the numerical
// errors when OPT__OUTPUT_USER is enabled
//                   --> In this case, it should provide the analytical solution
//                   at the given "Time"
//                2. This function will be invoked by multiple OpenMP threads
//                when OPENMP is enabled
//                   --> Please ensure that everything here is thread-safe
//                3. Even when DUAL_ENERGY is adopted for HYDRO, one does NOT
//                need to set the dual-energy variable here
...
void SetGridIC(real fluid[], const double x, const double y, const double z, const double Time, const int lv, double AuxArray[]) 
{
  ...
  Pres = EoS_DensTemp2Pres_CPUPtr(Dens, ISO_TEMP, NULL, EoS_AuxArray_Flt, EoS_AuxArray_Int, h_EoS_Table);
...
//
// Note        :  1. This function will be invoked by multiple OpenMP threads
// when OPENMP is enabled
...
// Return      :  magnetic
//-------------------------------------------------------------------------------------------------------
void SetBFieldIC(real magnetic[], const double x, const double y, const double z, const double Time, const int lv, double AuxArray[]) 
{
...
#ifdef PARTICLE
void Par_Init_ByFunction_SinkParTest(const long NPar_ThisRank, const long NPar_AllRank, real_par *ParMass,
                                     real_par *ParPosX, real_par *ParPosY, real_par *ParPosZ, real_par *ParVelX,
                                     real_par *ParVelY, real_par *ParVelZ, real_par *ParTime, long_par *ParType,
