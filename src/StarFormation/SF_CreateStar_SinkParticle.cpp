...
// Description :  Check whether the gas cell is close to any existing particle
// and satisfies the second density threshold
//
...
bool ProximityCheck_SecondDenThres(const int ExistingNPar, real *ParAtt_Local[],
                                   const real PosX, const real PosY,
                                   ...
                                   const real Coeff_FreeFall) 
{
  ...
  for (int p = 0; p < ExistingNPar; p++) // loop over all nearby existing particles
  {
    ...
    if (Par2CellVel[0] / NorPar2Cell[0] >= 0)
      continue;
    if (Par2CellVel[1] / NorPar2Cell[1] >= 0)
      continue;
    if (Par2CellVel[2] / NorPar2Cell[2] >= 0)
      continue; // the gas is moving away from the existing particle
...
//-------------------------------------------------------------------------------------------------------
// Function    :  GravMini
// Description :  Check whether the gas cell has the minimum gravitational
...
bool GravMini(const int pi, const int pj, const int pk, const real AccCellNum, const real *Pot_Array_F, const int Size_Flu, const real Phi000) 
{
  ...
  {
    if (SQRT(SQR(vi - pi) + SQR(vj - pj) + SQR(vk - pk)) > AccCellNum)
      continue;
...
// Description :  Check whether the gas in the nearby six cells converge toward
// the central one
//
// Parameter   :  pi, pj, pk      : Indices of the current cell
//                Size_Flu        : Size of the fluid array
...
bool JeansInstability_BoundState(const int pi, const int pj, const int pk, const real AccCellNum, const int Size_Flu, const real (*Flu_Array_F_In)[Size_Flu][Size_Flu][Size_Flu], const real *Mag_Array_F_In, const int Mag_Stride, const real dv, const real dh) 
{
  ...
  for (int vi = pi - AccCellNum; vi <= pi + AccCellNum; vi++) 
  {
    ...
    for (int v = 0; v < NSinkVar; v++)
      ControlFluid[v] = Flu_Array_F_In[v][vk][vj][vi];

    MassVel[0] += ControlFluid[SINK_DENS] * ControlFluid[SINK_VELX] * dv;
    MassVel[1] += ControlFluid[SINK_DENS] * ControlFluid[SINK_VELY] * dv;
    MassVel[2] += ControlFluid[SINK_DENS] * ControlFluid[SINK_VELZ] * dv;
    TotalMass += ControlFluid[SINK_DENS] * dv;
  }
  ...
  for (int vi = pi - AccCellNum; vi <= pi + AccCellNum; vi++) 
  {
    ...
    for (int vij = pi - AccCellNum; vij <= pi + AccCellNum; vij++) 
    {
      ...
      SelfPhiijk += -NEWTON_G * ControlFluidj[SINK_DENS] * dv / rij;
    }

    Egtot += 0.5 * ControlFluid[SINK_DENS] * dv * SelfPhiijk;

    // Storing Emagtot
#   ifdef MHD
    vEmag = MHD_GetCellCenteredBEnergy(Mag_Array_F_In + MAGX * Mag_Stride, Mag_Array_F_In + MAGY * Mag_Stride, Mag_Array_F_In + MAGZ * Mag_Stride, Size_Flu, Size_Flu, Size_Flu, vi, vj, vk);
    Emagtot += vEmag * dv;
#   endif

    // Storing Ethtot and Ekintot
    Pres = EoS_DensEint2Pres_CPUPtr(ControlFluid[SINK_DENS], ControlFluid[SINK_EINT], NULL, EoS_AuxArray_Flt, EoS_AuxArray_Int, h_EoS_Table);
    Cs2 = EoS_DensPres2CSqr_CPUPtr(ControlFluid[SINK_DENS], Pres, NULL, EoS_AuxArray_Flt, EoS_AuxArray_Int, h_EoS_Table);

    Ethtot += 0.5 * ControlFluid[SINK_DENS] * dv * Cs2;

    Ekintot += 0.5 * ControlFluid[SINK_DENS] * dv * (SQR(ControlFluid[SINK_VELX] - BulkVel[0]) + SQR(ControlFluid[SINK_VELY] - BulkVel[1]) + SQR(ControlFluid[SINK_VELZ] - BulkVel[2]));
  }
...
//                2. One must turn on STORE_POT_GHOST when adopting
//                STORE_PAR_ACC
//                   --> It is because, currently, this function always uses the
//                   pot_ext[] array of each patch
//                       to calculate the gravitationally acceleration of the
//                       new star particles
//                3. One must invoke Buf_GetBufferData( ..., _TOTAL, ... )
//                "before" calling this function
//
// Parameter   :  lv           : Target refinement level
//                TimeNew      : Current physical time (after advancing solution
//                by dt) GasDensThres : Minimum gas density for creating sink
//                particles                (--> "SF_CREATE_SINK_MIN_GAS_DENS"  )
//                AccCellNum   : Accretion radius in cells at highest refinement
//                level          (--> "SF_CREATE_SINK_ACC_RADIUS"    ) MaxNewPar
//                : Maximum number of particle per MPI rank (-->
...
void SF_CreateStar_SinkParticle(const int lv, const real TimeNew, const real GasDensThres, const real AccCellNum, const int MaxNewPar) 
{
  // check
...
# endif
  ...
  Par_CollectParticle2OneLevel(lv, ParAttFltBitIdx_In, ParAttIntBitIdx_In, PredictPos_No, TimeNew, SibBufPatch_Yes, FaSibBufPatch_No, JustCountNPar_No, TimingSendPar_Yes);
...
#     else
      real *Mag_Array = NULL;
#     endif
      Prepare_PatchData(lv, TimeNew, (real *)Flu_Array_F_In, Mag_Array, NGhost,
                        NPG, &PID0, _DENS | _VELX | _VELY | _VELZ | _EINT, _MAG,
                        OPT__FLU_INT_SCHEME, OPT__MAG_INT_SCHEME,
                        UNIT_PATCHGROUP, NSIDE_26, IntPhase_No, OPT__BC_FLU,
                        BC_POT_NONE, MinDens, MinPres_No, MinTemp_No,
                        MinEntr_No, DE_Consistency);

      // prepare the potential array
      if (OPT__SELF_GRAVITY || OPT__EXT_POT)
        Prepare_PatchData(lv, TimeNew, Pot_Array_F, NULL, NGhost, NPG, &PID0,
                          _POTE, _NONE, OPT__GRA_INT_SCHEME, INT_NONE,
                          UNIT_PATCHGROUP, NSIDE_26, IntPhase_No, OPT__BC_FLU,
                          OPT__BC_POT, MinDens_No, MinPres_No, MinTemp_No,
                          MinEntr_No, DE_Consistency_No);

      // prepare the corner array
      for (int d = 0; d < 3; d++)
        Corner_Array_F[d] = amr->patch[0][lv][PID0]->EdgeL[d] + 0.5 * dh - dh * NGhost;
      ...
      if (NParAllPatch > 0) {
        for (int v = 0; v < PAR_NATT_FLT_TOTAL; v++)
          if (ParAttFltBitIdx_In & BIDX(v))
            ParAtt_Local[v] = new real[NParAllPatch];
...
#           endif
      ...
      for (int pi = NGhost; pi < PS2 + NGhost; pi++) 
      {
        ...
        // We can use the distance between the gas cell and nearby
        // particle and the relative velocity to define a a density
        // threshold (Clarke et al. 2017, eqn (5))
        // ============================================================================================================
        if (!ProximityCheck_SecondDenThres(
                ExistingNPar, ParAtt_Local, PosX, PosY, PosZ, VelX, VelY,
                VelZ, GasDens, AccRadius, Coeff_FreeFall))
          continue;
        ...
        // ============================================================================================================
        if (!JeansInstability_BoundState(pi, pj, pk, AccCellNum, Size_Flu, Flu_Array_F_In, (real *)Mag_Array_F_In, Size_Flu_P1 * SQR(Size_Flu), dv, dh))
          continue;
        ...
        # pragma omp critical
        {
              ...
              if (OPT__SELF_GRAVITY || OPT__EXT_POT) 
              {
                for (int NeighborID = 0; NeighborID < 6; NeighborID++) {
                  if (NeighborID == 0)
                    delta_t = IDX321(1, 0, 0, Size_Flu, Size_Flu);
                  else if (NeighborID == 1)
                    delta_t = IDX321(-1, 0, 0, Size_Flu, Size_Flu);
                  else if (NeighborID == 2)
                    delta_t = IDX321(0, 1, 0, Size_Flu, Size_Flu);
                  else if (NeighborID == 3)
                    delta_t = IDX321(0, -1, 0, Size_Flu, Size_Flu);
                  else if (NeighborID == 4)
                    delta_t = IDX321(0, 0, 1, Size_Flu, Size_Flu);
                  else if (NeighborID == 5)
                    delta_t = IDX321(0, 0, -1, Size_Flu, Size_Flu);
  ...
  disp[0] = 0;
  for (int rank = 1; rank < MPI_NRank; rank++)
  ...
  MPI_Allgatherv(RemovalFlu[0], SendRemovalFluSize, MPI_GAMER_REAL, GatherRemovalFlu[0], RecvRemovalFluSize, disp, MPI_GAMER_REAL, MPI_COMM_WORLD);
...
# ifdef DEBUG_PARTICLE
    // do not set ParPos too early since pointers to the particle repository (e.g., amr->Par->PosX) may change after calling amr->Par->AddOneParticle()
    const real_par *NewParPos[3] = {amr->Par->PosX, amr->Par->PosY, amr->Par->PosZ};
    char Comment[100];
    sprintf(Comment, "%s", __FUNCTION__);

    amr->patch[0][lv][SPID]->AddParticle(NParInPatch, ParRepoInPatch, &amr->Par->NPar_Lv[lv], PType, NewParPos, amr->Par->NPar_AcPlusInac, Comment);
